...
--   destination_number  varchar(25)  -- HostMobile, formatted to +61
--   content             varchar(1024) -- TextMessageBody with tokens merged
--
-- INTERNAL CALLS:
--   [SMSGateway].[GET_UNTRANSMITTED_NEW_MEMBER_MM]   -- feed 1
--   [SMSGateway].[GET_UNTRANSMITTED_TIER_UPGRADE_MM] -- feed 2
--   Inline logic for PlayerOffers_SMS               -- feed 3
--
-- NOTE: This SP is a waterfall dispatcher. New Member takes priority
--       over Tier Upgrade which takes priority over Bonus Award.
--       This causes starvation of lower-priority feeds.
--       Rewrite will separate into three independent per-feed SPs.
-- ============================================================
...
BEGIN
    ...
    IF EXISTS (
        SELECT * FROM [SMSGateway].[NewMember_Host_SMS]
        WHERE iseligibleforsms = 1
          AND ISNULL(intransmission, 0) = 0
          AND ISNULL(SubmittedToHostSMS, 0) = 0
    )
    BEGIN
    ...
    IF EXISTS (
        SELECT * FROM [SMSGateway].[TierUpgrades_Host_SMS]
        WHERE iseligibleforsms = 1
          AND ISNULL(intransmission, 0) = 0
    )
    BEGIN
    ...
    SELECT @playeroffers_smsid = posms.PlayerOffers_SMSID,
           @VenueID = posms.VenueID
    ...
    ) tp ON posms.PlayerOffers_SMSID = tp.PlayerOffers_SMSID
         AND posms.venueid = tp.venueid

    PRINT @playeroffers_smsid
    PRINT @venueid
    ...
    -- PE HOST GUARD: Only one bonus SMS per day per PE host
    IF EXISTS (
        SELECT 1 FROM [SMSGateway].ThirdPartySMSIDS
        WHERE HostThirdPartyID = @hostID AND ISNULL(ispehost, 0) = 1 AND VenueID = @VenueID
    )
    BEGIN
        IF EXISTS (
            SELECT 1 FROM [SMSGateway].PlayerOffers_SMS
            WHERE VenueID = @VenueID AND ThirdPartyHostID = @hostID AND InTransmission = 1
              AND SubmittedToHostSMSDateTime > dbo.current_day_start(GETDATE())
              AND ISNULL(SuppressedFromTransmission, 0) = 0
        )
        BEGIN
            UPDATE [SMSGateway].PlayerOffers_SMS
            SET IsEligibleForSMS = 0, SubmittedToHostSMS = 0,
                SubmittedToHostSMSDateTime = GETDATE(), SuppressedFromTransmission = 1,
                SuppressedFromTransmissionDateTime = GETDATE()
            WHERE PlayerOffers_SMSID = @PlayerOffers_SMSID AND VenueID = @VenueID
            PRINT 'Message Suppressed - PE Host Already Transmitted To Today'
            RETURN
        END
    END
